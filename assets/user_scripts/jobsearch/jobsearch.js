...
var Invsum=0;
    ...
    $.each(result["invoicedata"], function(index, value){
       ...
       var stringval='<ul class="nav"><li class="dropdown"> <a class="btn btn-sm dropdown-toggle" style="width: 50px;" data-toggle="dropdown" href="#"><i class="fa fa-ellipsis-v"></i></a><ul class="dropdown-menu">';          
        ...
        //  }
         ...
         $(".dataadd").append( "<tr class='tbl_row'><td class='sl'>"+slno+" </td> <td class='inv'>"+inv+"</td><td class='date'>"+date+"</td>  <td class='mode'>"+mode+"</td>  <td class='subtotal'>"+subtotal+"</td> <td class='vattotal'>"+vattotal+"</td><td class='grandtotal'>"+grandtotal+"</td><td>"+stringval+" </td></tr>" );
   ...
   console.log(Invsum);
    ...
    $.each(result["receiptdata"], function(index, value){
      ...
      //  var amount=value.SubTotal;
       ...
       receipt=parseFloat(receipt)+parseFloat(amount);
        ...
        $(".paymentreceipt").append( "<tr class='tbl_row'><td class='sl'>"+slno+" </td> <td class='date'>"+date+"</td> <td class='doc'>"+doc+"</td> <td class='particulars'>"+particulars+"</td> <td class='invoice'>"+invoice+"</td>  <td class='amount'>"+amount+"</td> </tr>" );

    });
    receipt=parseFloat(receipt).toFixed(2);
    $(".paymentreceipt").append( "<tr class='tbl_row' style='background-color: #e6e6ff;'><td colspan='7' > Total Receipt </td><td style='text-align: right;'>"+receipt+"</td></tr>"); 
    ...
    $.each(result["expense"], function(index, value){
     ...
     //  }
      stringval=stringval+'</ul> </li>  </ul>';
        $(".postedexpense").append( "<tr class='tbl_row'><td class='sl'>"+slno+" </td> <td class='date'>"+date+"</td> <td class='doc'>"+doc+"</td><td class='particulars'>"+particulars+"</td>  <td class='invoice'>"+invoice+"</td> <td class='supplier'>"+supplier+"</td> <td class='amount' colspan='2'>"+amount+"</td><td>"+stringval+" </td></tr>" );

    });
    Expense=parseFloat(Expense).toFixed(2);
    $(".postedexpense").append( "<tr class='tbl_row' style='background-color: #e6e6ff;'><td colspan='7' > Total Expense</td><td style='text-align: right;'>"+Expense+"</td></tr>"); 
    ...
    $.each(result["debitnotedata"], function(index, value){
      ...
      // total=total+amount;
        $(".debitnote").append( "<tr class='tbl_row'><td class='sl'>"+slno+" </td> <td class='date'>"+date+"</td> <td class='doc'>"+doc+"</td> <td class='invoice'>"+invoice+"</td> <td class='supplier'>"+supplier+"</td> <td class='amount'>"+amount+"</td> </tr>" );

    });
    debitnote=parseFloat(debitnote).toFixed(2);
    $(".debitnote").append( "<tr class='tbl_row' style='background-color: #e6e6ff;'><td colspan='7' > Total Debitnote </td><td style='text-align: right;'>"+debitnote+"</td></tr>"); 
    //  console.log(total)
    var suppayment=0;
    var slno=0;
    $.each(result["supplierpayment"], function(index, value){
      ...
      suppayment=parseFloat(suppayment)+parseFloat(amount);
        $(".payment").append( "<tr class='tbl_row'><td class='sl'>"+slno+" </td> <td class='date'>"+date+"</td> <td class='doc'>"+doc+"</td> <td class='particulars'>"+particulars+"</td><td class='supplier'>"+supplier+"</td> <td class='amount'>"+amount+"</td> </tr>" );

    });
    suppayment=parseFloat(suppayment).toFixed(2);
    $(".payment").append( "<tr class='tbl_row' style='background-color: #e6e6ff;'><td colspan='7' > Total Supplier Payment </td><td style='text-align: right;'>"+suppayment+"</td></tr>"); 
